...
Lütfen kullanıcıya kibar, net ve bilgilendirici bir Türkçe yanıt ver. Yanıtında ses içeriğindeki bilgilere sadık kal.`;
    ...
    // 🌐 MOD: AI ÇOKLU DİL ÇEVİRİSİ (TRANSLATION)
    if (mode === "cevir") {
      const { rapor, hedefDil } = body;
      if (!rapor || !hedefDil) {
        return NextResponse.json({ error: "Çeviri için rapor verisi ve hedef dil gereklidir." }, { status: 400 });
      }

      const cevirPrompt = `Sen profesyonel bir çevirmen ve dil uzmanısın. Aşağıda Türkçe yazılmış bir ses analiz raporu yer alıyor.
Senden bu rapordaki bilgileri anlam kaybı olmadan, doğal ve akıcı bir şekilde "${hedefDil}" diline çevirmeni istiyorum.
Sadece saf bir JSON çıktısı ver. Markdown kod blokları (\`\`\`json gibi) ekleme.

Girdi Rapor Verisi:
${JSON.stringify(rapor, null, 2)}

İstenen JSON Çıktı Formatı:
{
  "metin": "Çevrilmiş ana özet metni...",
  "actionItems": ["Çevrilmiş görev 1", "Çevrilmiş görev 2"],
  "kritikYerler": ["Çevrilmiş kritik nokta 1", "Çevrilmiş kritik nokta 2"],
  "sinavdaCikabilir": ["Çevrilmiş sınav konusu 1", "Çevrilmiş sınav konusu 2"],
  "modAnalizi": ["Çevrilmiş ruh hali maddesi 1", "Çevrilmiş ruh hali maddesi 2"],
  "sentiment": "Çevrilmiş duygu etiketi",
  "tone": "Çevrilmiş ton etiketi"
}`;

      try {
        const cevirResponse = await ai.models.generateContent({
          model: "gemini-flash-latest",
          contents: [
            {
              role: "user",
